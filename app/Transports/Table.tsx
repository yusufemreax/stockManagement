...
export default function DoTable() {
    ...
    const [data, setData] = useState<TransportDetail[]>([]);
    ...
    const fetchData = async () => {
      try {
        const response: AxiosResponse<TransportDetail[]> = await axios.get('http://46.8.194.136/stocktest/Transport/GetAllDetail');
    ...
    const deleteRow =async (rowID:string) => {
      try {
        await axios.post('http://46.8.194.136/stocktest/Transport/DeleteById',rowID,{headers: {
    ...
    const deleteRows =async (rowIDs:string[]) => {
      try {
        ...
        await axios.post('http://46.8.194.136/stocktest/Transport/DeleteMany',rowIDs,{headers: {
          'Content-Type': 'application/json',
          'Access-Control-Allow-Origin': 'http://localhost:3000' // Bu satırı ekleyin veya güncelleyin
        }});
        fetchData();
    ...
    return (
      <div className='flex'>
          ...
          {ismodalOpen && createPortal(<TransportModal isOpen={ismodalOpen} onClose={()=>setIsmodalOpen(false)} onSubmit={()=>setIsModalSubmitted(true)} rowID={modalRowId} />,document.body)}
      </div>
    )
}